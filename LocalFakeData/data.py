...
def searchByName(name):
    try:
        db = pymysql.connect(host=config.DB_HOST, port=int(config.PORT), user=config.DB_USER, passwd=config.DB_PASSWORD, db=config.DB_SCHEMA, charset='utf8')
        # conn = pymysql.connect(**db_settings)
        with db.cursor() as cursor:
            #  command = "SELECT * FROM info WHERE name = %s"
             command = "SELECT * FROM info WHERE name = 勞保傷病給付申辦"
...
support = {"生育及育兒": ["育有未滿2歲兒童育兒津貼","桃園市生育津貼","農民健康保險生育給付申辦"],
"長者": ["勞保老年給付申辦（個人申辦用）","勞保老年給付申辦（個人申辦用）"]}
